...
async function testDiscoveryModule() {
  ...
  // 5. Test Poster DOM rendering with inside title & gradient
  ...
  const renderedPosterHtml = window.home.createItem(continueItems[0]);
  assert(renderedPosterHtml.includes("poster-overlay-gradient"), "Poster must include gradient overlay");
  assert(renderedPosterHtml.includes("poster-inner-meta"), "Poster must include inner metadata container");
  assert(renderedPosterHtml.includes("poster-inner-title"), "Poster must include title inside the poster");
  assert(renderedPosterHtml.includes("progress-track"), "Poster must include full-width progress track");
  assert(renderedPosterHtml.includes("progress-fill"), "Poster must include progress fill");
